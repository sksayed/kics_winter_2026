...
Recent surveys highlight the storage and performance challenges of PQC adoption in blockchain systems \cite{wang2025review}. While Dilithium-3 provides strong security guarantees, its signature size creates significant scalability issues. Following the benchmarks in \cite{wang2025review}, Falcon-512 offers faster verification (35--45\% improvement) with smaller signatures, making it attractive for high-frequency applications. Hybrid approaches combining ECDSA with Dilithium maintain backward compatibility but double storage requirements when applied to every transaction \cite{wang2025review}.

This paper proposes a novel \textit{\textbf{Off-Chain Hybrid ZKP Framework}} that addresses these challenges through three key contributions: (1) a two-layer architecture combining HCF for on-chain settlement with scriptless adaptor signatures for off-chain scaling, minimizing on-chain data footprint; (2) replacement of lattice-based witness proofs with hash-based ZKPs (MPC-in-the-head), mitigating side-channel vulnerabilities while ensuring quantum resistance; and (3) a framework that achieves 99\% storage reduction compared to on-chain PQC while maintaining quantum resistance and side-channel security.
...
\textbf{The Improvement (Hash-Based ZKP):} Current LAS \cite{esgin2020adaptor} suffer from a \textit{\textbf{knowledge gap}} and side-channel vulnerabilities. We replace the lattice witness proof with MPC-in-the-head (as used in the Picnic scheme) \cite{buser2023survey}. \textbf{Construction:} The pre-signature binds to the MPC-in-the-head proof through a commitment scheme: the witness $w$ is committed via $C = \text{Hash}(w || r)$ where $r$ is a random nonce. The MPC-in-the-head circuit proves knowledge of $w$ such that $C$ opens correctly, and the adaptor signature embeds this commitment into the signature structure. This leverages hash-based commitments rather than group operations, enabling adaptor signature functionality with symmetric primitives.

\textbf{Justification:} MPC-in-the-head relies solely on symmetric primitives (SHA-256, AES), avoiding Gaussian sampling and complex arithmetic that render Lattices vulnerable to timing attacks. Unlike lattice-based ZKPs that require rejection sampling and polynomial operations susceptible to power analysis, hash-based proofs execute in constant time, ensuring quantum security and side-channel resistance. This addresses the fundamental limitation of LAS where weak pre-signature adaptability prevents reliable witness extraction. \textbf{Trade-off Acknowledgment:} Following the benchmarks in \cite{buser2023survey}, hash-based ZKPs (e.g., Picnic) produce larger proof sizes (10--50 KB) compared to lattice-based proofs, but remain off-chain and are only exchanged between channel participants. Only a compact hash commitment or dispute resolution signature reaches the ledger during channel closure, ensuring on-chain storage efficiency.
...
By restricting HCF dual-signatures to the settlement layer, our framework bypasses linear storage growth. \textbf{Comparison with Classical Layer 2:} Following the benchmarks in \cite{wang2025review}, a classical Layer 2 (e.g., Lightning Network) with ECDSA requires approximately 0.07 KB per transaction. Our PQC-Layer 2 requires 3.5 KB per settlement (dual-signature: ECDSA + Dilithium) \cite{wang2025review}. For 5,000 transactions, only 1 Settlement Transaction is stored ($\approx$ 3.5 KB) instead of storing all individual transactions. The off-chain design enables unlimited transaction throughput limited only by local bandwidth.

\subsection{Computational Efficiency}

The framework leverages Falcon-512 for verification. Falcon verifies significantly faster than Dilithium and ECC \cite{wang2025review}, while Kyber's 15--20\% handshake overhead is amortized over the channel lifetime \cite{wang2025review}. Off-chain transactions utilize hash-based ZKP verification ($\sim$2--5 ms) \cite{buser2023survey}, acceptable for real-time processing.

\textbf{IoT Feasibility:} Falcon-512 verification consumes 0.09 mJ on constrained devices \cite{wang2025review}, making our framework viable for battery-powered IoT nodes. The off-chain design further reduces energy consumption by minimizing blockchain interactions.

\subsection{Security Resilience}

This framework addresses the \textit{\textbf{Single Point of Failure}} risk through cryptographic diversity. \textbf{Cryptographic Agility:} The off-chain ZKP relies on Hash-based assumptions (Picnic/MPC-in-the-head), while the on-chain anchor relies on Lattice assumptions (Kyber/Dilithium) and Classical assumptions (ECC). This multi-assumption approach ensures that a break in one cryptographic primitive does not compromise the entire system. \textbf{Side-Channel Defense:} Hash-based proofs execute in constant time, eliminating variable execution paths that enable timing attacks on lattice sampling operations. This is particularly critical in IoT environments where devices may lack hardware security modules.

\section{Conclusion}

The \textit{\textbf{Off-Chain Hybrid ZKP Framework}} offers a pragmatic path to Post-Quantum standardization. By synthesizing Adaptor Signatures with the HCF, we resolve the conflict between quantum security and blockchain scalability. Hash-based ZKPs address the side-channel vulnerabilities and \textit{\textbf{knowledge gaps}} in current Lattice implementations, providing a robust, efficient solution for decentralized finance.

Our framework achieves 99\% storage reduction while maintaining quantum resistance through hybrid cryptography and side-channel security through hash-based proofs. The architecture is feasible for IoT deployment with verification times under 1 ms and minimal energy consumption. Future work will focus on optimizing ZKP proof sizes, formal security proofs for the hash-based adaptor signature construction, and hardware acceleration for resource-constrained devices.
